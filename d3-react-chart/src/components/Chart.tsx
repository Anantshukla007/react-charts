...
const Chart = () => {
  ...
  useEffect(() => {
    ...
    // Draw Axes with smooth transitions
    ...
    svg
      ...
      .on("mouseover", function (event, d) {
        ...
        tooltip
          ...
          .style("top", `${barY - 10}px`); // 10px above the bar
      })
    ...
    svg
      ...
      .attr("y", (d) => yScale(d.sales) - 5) // 5px above the bar
    ...
    // Legend: Centered at the top of the chart
  ...
  // Updated addDataPoint: update data for the current month if it exists, otherwise append new data.
  const addDataPoint = () => {
    const newMonth = new Date().toLocaleString("en-US", { month: "short" });
    const newSales = Math.floor(Math.random() * 400) + 100;
    const monthExists = data.find((d) => d.month === newMonth);
    if (monthExists) {
      // Update the existing data point with the new sales value.
      setData(data.map((d) => (d.month === newMonth ? { ...d, sales: newSales } : d)));
    } else {
      // Append the new data point.
      setData([...data, { month: newMonth, sales: newSales }]);
    }
  };

  const filterData = () => {
    setData(data.filter((d) => d.sales >= 200));
  };
